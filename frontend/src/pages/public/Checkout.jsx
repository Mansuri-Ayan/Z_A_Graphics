...
const AddressForm = ({ register, prefix, errors }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
      ...
      <input {...register(`${prefix}.fullName`)} type="text" className={`w-full border ${errors?.fullName ? 'border-red-500' : 'border-gray-200'} rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500`} />
      {errors?.fullName && <p className="text-red-500 text-xs mt-1">{errors.fullName.message}</p>}
    </div>
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-1">Company Name</label>
      <input {...register(`${prefix}.companyName`)} type="text" className="w-full border border-gray-200 rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500" />
    </div>
    <div className="md:col-span-2">
      <label className="block text-sm font-medium text-gray-700 mb-1">Email Address *</label>
      <input {...register(`${prefix}.email`)} type="email" className={`w-full border ${errors?.email ? 'border-red-500' : 'border-gray-200'} rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500`} />
      {errors?.email && <p className="text-red-500 text-xs mt-1">{errors.email.message}</p>}
    </div>
    <div className="md:col-span-2">
      <label className="block text-sm font-medium text-gray-700 mb-1">Street Address *</label>
      <input {...register(`${prefix}.streetAddress`)} type="text" className={`w-full border ${errors?.streetAddress ? 'border-red-500' : 'border-gray-200'} rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500`} />
      {errors?.streetAddress && <p className="text-red-500 text-xs mt-1">{errors.streetAddress.message}</p>}
    </div>
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-1">City *</label>
      <input {...register(`${prefix}.city`)} type="text" className={`w-full border ${errors?.city ? 'border-red-500' : 'border-gray-200'} rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500`} />
      {errors?.city && <p className="text-red-500 text-xs mt-1">{errors.city.message}</p>}
    </div>
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-1">State *</label>
      <input {...register(`${prefix}.state`)} type="text" className={`w-full border ${errors?.state ? 'border-red-500' : 'border-gray-200'} rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500`} />
      {errors?.state && <p className="text-red-500 text-xs mt-1">{errors.state.message}</p>}
    </div>
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-1">PIN Code *</label>
      <input {...register(`${prefix}.pinCode`)} type="text" className={`w-full border ${errors?.pinCode ? 'border-red-500' : 'border-gray-200'} rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500`} />
      {errors?.pinCode && <p className="text-red-500 text-xs mt-1">{errors.pinCode.message}</p>}
    </div>
    <div>
      <label className="block text-sm font-medium text-gray-700 mb-1">Phone Number *</label>
      <input {...register(`${prefix}.phone`)} type="tel" className={`w-full border ${errors?.phone ? 'border-red-500' : 'border-gray-200'} rounded px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500`} />
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-3/5">
            ...
            <div className="mb-8">
              <div className="flex items-center">
                <div className={`flex items-center ${step >= 1 ? 'text-blue-600' : 'text-gray-400'}`}>
                  <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold border-2 ${step >= 1 ? 'border-blue-600 bg-blue-50' : 'border-gray-200'}`}>1</span>
                ...
                <div className={`flex items-center ${step >= 2 ? 'text-blue-600' : 'text-gray-400'}`}>
                  <span className={`w-8 h-8 rounded-full flex items-center justify-center font-bold border-2 ${step >= 2 ? 'border-blue-600 bg-blue-50' : 'border-gray-200'}`}>2</span>
            ...
            {step === 1 && (
              <form onSubmit={handleSubmit(onShippingSubmit)} className="bg-white rounded-3xl border border-gray-100 p-6 md:p-10 shadow-sm">
                <h2 className="text-2xl font-black text-gray-900 mb-6">Billing Address</h2>
            ...
            {step === 2 && (
              <div className="bg-white rounded-3xl border border-gray-100 p-6 md:p-10 shadow-sm">
                ...
                <div className="border-2 border-blue-600 bg-blue-50 rounded-2xl p-6 mb-8 flex items-start">
              ...
              <div className="space-y-6 mb-8 max-h-[40vh] overflow-y-auto pr-2">
                {cart.map((item) => (
                  <div key={item.id} className="flex items-start">
                    <div className="w-20 h-20 bg-gray-50 border border-gray-100 rounded-xl flex-shrink-0 overflow-hidden">
